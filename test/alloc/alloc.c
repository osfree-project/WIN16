...
static void test_Local(void)
{
    ...
    ok(mem1 != (HLOCAL)NULL, "LocalAlloc failed: error=%d\n", GetLastError());
    if (mem1) {
    ...
    ok(mem2 != (HLOCAL)NULL, "LocalAlloc failed: error=%d\n", GetLastError());
    if (mem2) {
    ...
    ok(mem2a == (HLOCAL)NULL, "LocalFree failed: %p\n", mem2a);

    /* Reallocate mem2 as moveable memory */
    mem2 = LocalAlloc(LMEM_MOVEABLE | LMEM_ZEROINIT, memchunk);
    ok(mem2 != (HLOCAL)NULL, "LocalAlloc failed to create moveable memory, error=%d\n",
       GetLastError());

    /* Check that ReAllocing memory works as expected */
    mem2a = LocalReAlloc(mem2, 2 * memchunk, LMEM_MOVEABLE | LMEM_ZEROINIT);
    ok(mem2a != (HLOCAL)NULL, "LocalReAlloc failed, error=%d\n", GetLastError());
    if (mem2a) {
