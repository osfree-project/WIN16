...
CopyToClipboard(LPTSTR pszFrom)
{
   ...
   uFormatW = RegisterClipboardFormat("FileNameW");
   uFormatLongW = RegisterClipboardFormat("LongFileNameW");
   uFormatA = RegisterClipboardFormat("FileName");
   uFormatLongA = RegisterClipboardFormat("LongFileName");
...
// (not localized)
TCHAR szFileVersion[]    = "FileVersion";
TCHAR szLegalCopyright[] = "LegalCopyright";
...
#undef GET_PROC
   ...
   //
   VerQueryValue(lpVersionBuffer, "\\VarFileInfo\\Translation",
   ...
   //
   wsprintf(szVersionKey, "\\StringFileInfo\\%04X04B0\\",
      LANGIDFROMLCID(lcid));
   ...
   if (cbValueTranslation != 0) {
      ...
      wsprintf(szVersionKey, "\\StringFileInfo\\%04X%04X\\",
         *lpXlate, *(lpXlate+1));
   ...
   //
   lstrcpy(szVersionKey, "\\StringFileInfo\\040904B0\\");
   lpszValue = GetVersionDatum(szFileVersion);
   ...
   // (04E4) here
   //
   lstrcpy(szVersionKey, "\\StringFileInfo\\040904E4\\");
   lpszValue = GetVersionDatum(szFileVersion);
   ...
   // (0000) here
   //
   lstrcpy(szVersionKey, "\\StringFileInfo\\04090000\\");
   lpszValue = GetVersionDatum(szFileVersion);
   ...
   if (WNetStat(NS_PROPERTYDLG)) {
      ...
      if (p) {
         ...
         for (i = 0; i < 6; i++) {
            ...
            hwnd = CreateWindowEx(0L, "button", szTemp,
