...
#if !defined(_COMCTL32_) && defined(TWIN32)
...
BOOL        WINAPI ImageList_DrawEx(HIMAGELIST,int,HDC,int,int,int,int,
		COLORREF , COLORREF , UINT );
BOOL        WINAPI ImageList_Remove(HIMAGELIST , int );
HICON       WINAPI ImageList_GetIcon(HIMAGELIST , int , UINT );

BOOL WINAPI ImageList_GetImageInfo (HIMAGELIST himl, int ImageIndex, 
		IMAGEINFO FAR *pImageInfo);
BOOL WINAPI ImageList_Draw( HIMAGELIST himl, int ImageIndex,HDC hDC, 
		int X, int Y, UINT DrawFlags);
HIMAGELIST WINAPI ImageList_Merge(HIMAGELIST himl1, int i1, HIMAGELIST himl2,
	 	int i2,	int dx, int dy);	
HIMAGELIST WINAPI ImageList_LoadBitmap(HINSTANCE hi, LPCSTR lpbmp, 	
		int cx, int cGrow, COLORREF crMask);
 
#define     ImageList_AddIcon(himl, hicon) ImageList_ReplaceIcon(himl, -1, hicon)

#endif /* NOAPIPROTO */
#endif /* _WIMAGELIST_SOURCE */
...
HIMAGELIST  WINAPI ImageList_GetDragImage(POINT FAR* ,POINT FAR* );
#endif /* _WIMAGELIST_SOURCE */
#endif
...
WINAPI ImageList_Write(HIMAGELIST himl, LPSTREAM pstm);
#endif
#endif
...
#define TBSTATE_HIGHLIGHTED     0x40
