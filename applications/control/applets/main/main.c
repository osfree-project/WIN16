...
BOOL CALLBACK FontsDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        HDC hdc = GetDC(hDlg);
        EnumFonts(hdc, (LPSTR)NULL, (FONTENUMPROC)EnumFontsProc, (LPARAM)hList);
        ReleaseDC(hDlg, hdc);
        return TRUE;
    }
    case WM_COMMAND:
...
BOOL CALLBACK MouseDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            wsprintf(buf, "%d", g_swapButtons);
            WriteProfileString("windows", "SwapButtons", buf);
            /* Сбросить флаги нажатий, чтобы избежать обработки после закрытия диалога */
            g_bLeftDown = FALSE;
            g_bRightDown = FALSE;
            g_pressedSwap = FALSE;
            EndDialog(hDlg, IDOK);
        ...
        case IDCANCEL:
            ...
            if (g_origSwap != GetSystemMetrics(SM_SWAPBUTTON))
                SwapMouseButton(g_origSwap);
            ApplyMouseSpeed(g_origSpeed);
            CheckDlgButton(hDlg, IDC_MS_SWAP, g_origSwap);
            UpdateButtonLabels(hDlg, g_origSwap);
            g_swapButtons = g_origSwap;
            /* Сбросить флаги нажатий */
            g_bLeftDown = FALSE;
            g_bRightDown = FALSE;
            g_pressedSwap = FALSE;
