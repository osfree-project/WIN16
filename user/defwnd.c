...
//@todo	return NC_HandleNCActivate( hwnd, wParam );
    ...
    case WM_NCDESTROY:
	if (wndPtr->hText) LocalFree(wndPtr->hText);
	if (wndPtr->hVScroll) LocalFree(wndPtr->hVScroll);
	if (wndPtr->hHScroll) LocalFree(wndPtr->hHScroll);
	...
	return WINPOS_HandleWindowPosChanging((WINDOWPOS FAR *)lParam);
    ...
    case WM_WINDOWPOSCHANGED:
	{
	    ...
	    WINDOWPOS FAR * winPos = (WINDOWPOS FAR *)lParam;
    ...
    case WM_GETTEXT:
	{
		...
		{
		    textPtr = LocalLock(wndPtr->hText);
		    if ((int)wParam > (len = lstrlen(textPtr)))
		    {
			lstrcpy((LPSTR)lParam, textPtr);
			return (DWORD)len;
		    }
		}
	    }
