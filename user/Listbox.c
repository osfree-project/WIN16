...
extern HBRUSH WINAPI GetControlBrush(HWND, HDC, WORD);
// @todo implement this!
...
LONG LBoxAPI(HMENU, UINT, LPARAM);
...
#define CREATELBOX(h) (LPLISTBOXINFO)GlobalAllocPtr(GPTR, sizeof(LISTBOXINFO))//(LPLISTBOXINFO)HandleObj(HM_GETHND,MLLBOX,(LPVOID)(&(h)))
#define FREELBOX(h) (GlobalFreePtr((LPLISTBOXINFO)h)) //HandleObj(HM_FREEHND,MLLBOX,(LPVOID)((DWORD)h))
...
//    HCLASS32 hLBoxClass32;
	...
	case LB_DIR:
	    if ( lParam != 0  &&  lstrlen((LPSTR)lParam) != 0 )
	    	{}// @todo Implement this!!! ListDir(hWnd,0,wParam,(LPSTR)lParam);
	    else
		return (LRESULT)LB_ERR;
	    return 1L;		/* TODO */
...
LBoxMeasureItem(LPLISTBOXINFO lpLBInfo, LPLISTBOXITEM lpLBItem, WORD wIndex)
{
    ...
    _fmemset((LPSTR)&mis,'\0',sizeof(MEASUREITEMSTRUCT));
    mis.itemData = 
...
LBoxGetSlot(LPLISTBOXINFO lpLBInfo)
{
	...
	if (!(lpLBInfo->lpItemList = (LPLISTBOXITEM far *)
		GlobalAllocPtr(GPTR,sizeof(LPLISTBOXITEM)*LPD_SLOTG))) {
	    return FALSE;
	}
	_fmemset((LPVOID)lpLBInfo->lpItemList, '\0',
	...
	if (!(lpLBInfo->lpItemList = (LPLISTBOXITEM far *)
		GlobalLock(GlobalReAlloc(GlobalPtrHandle((LPSTR)lpLBInfo->lpItemList),
		sizeof(LPLISTBOXITEM)*(lpLBInfo->wSlotCount+LPD_SLOTG), GMEM_MOVEABLE)))) {
	    return FALSE;
	}
	_fmemset((LPVOID)&lpLBInfo->lpItemList[lpLBInfo->wSlotCount], '\0',
		sizeof(LPLISTBOXITEM)*LPD_SLOTG);
	lpLBInfo->wSlotCount += LPD_SLOTG;
    }
    return TRUE;
...
LBoxResetContent(LPLISTBOXINFO lpLBInfo, BOOL bDeleteData)
{
	...
	if (bOwnData) {
		...
		GlobalFreePtr(lpLBItem->lpItemString);
	GlobalFreePtr((LPSTR)lpLBItem);
      }
      else {
	...
	GlobalFreePtr((LPSTR)lpLBInfo->lpItemList);
...
LBoxCaseComparePrefix(HWND hDest, LPCOMPAREITEMSTRUCT lpcis)
{
    ...
    nLen1 = lstrlen((LPSTR)(lpcis->itemData1));
    nLen2 = lstrlen((LPSTR)(lpcis->itemData2));
    ...
    return _fstrnicmp((LPSTR)(lpcis->itemData1),
...
LBoxModifyItem(LPLISTBOXINFO lpLBInfo, LPMENUITEMSTRUCT lpMis)
{
	...
	if (lpMis->wAction & LCA_SET) {
		...
		if ((int)lstrlen(lpMis->lpItemData) > 
				(int)lstrlen(lpLBItem->lpItemString))
		    lpLBItem->lpItemString = GlobalLock(GlobalReAlloc(GlobalPtrHandle(
			lpLBItem->lpItemString),
			lstrlen(lpMis->lpItemData)+1, GMEM_MOVEABLE));
		lstrcpy(lpLBItem->lpItemString,lpMis->lpItemData);
	    }
	    else
		...
		DWORD lParam, int nFunc)
{
	...
	case LB_GETTEXTLEN:
	    ...
	    if (nFunc == LB_GETTEXT) {
		if (!bOwnData)
		    lstrcpy((LPSTR)lParam, lpLBItem->lpItemString);
		else {
		    _fmemcpy((LPSTR)lParam,
		...
		return (LONG)(lstrlen(lpLBItem->lpItemString));
    }
    return 0;
}
		...
		LPMENUITEMSTRUCT lpMis)
{
    ...
    lpNewItem = (LPLISTBOXITEM)GlobalAllocPtr(GPTR, sizeof(LISTBOXITEM));
    _fmemset((LPVOID)lpNewItem, '\0', sizeof(LISTBOXITEM));
	...
	if (!HIWORD(lpMis->lpItemData) || (lpMis->wItemFlags & MF_BITMAP))
		...
		GlobalAllocPtr(GPTR, lstrlen(lpMis->lpItemData) + 1);
	    lstrcpy(lpNewItem->lpItemString, lpMis->lpItemData);
	}
    }
    else
...
*/
LONG
LBoxAPI(HMENU hMenu32, UINT uiAction, LPARAM lParam)
{
