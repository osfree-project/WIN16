 ...
 */
void GRAPH_DrawReliefRect( HDC hdc, RECT FAR *rect, int highlight_size,
...
static void PB_Paint( WND *wndPtr, HDC hDC, WORD action )
{
    ...
    text = (char*) LocalLock( wndPtr->hText );
    if (text && text[0])
    {
        ...
        {
            short xdelta, ydelta;
            dwTextSize = GetTextExtent( hDC, text, lstrlen(text) );
...
// Замена вызова GRAPH_DrawBitmap для рисования части битмапа (чекбокса) из общего изображения
if (hbitmapCheckBoxes != NULL)
{
    HDC hdcMem = CreateCompatibleDC(hDC);
    HGDIOBJ hOldBitmap = SelectObject(hdcMem, hbitmapCheckBoxes);
    BitBlt(hDC,
           rc.left,                             // x назначения
           rc.top + delta,                       // y назначения
           checkBoxWidth,                         // ширина
           checkBoxHeight,                        // высота
           hdcMem,
           x,                                     // исходный x в битмапе
           y,                                     // исходный y в битмапе
           SRCCOPY);
...
//                      x, y, checkBoxWidth, checkBoxHeight );
    ...
    if (!(text = (char*) LocalLock( wndPtr->hText ))) return;
