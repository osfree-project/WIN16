...
// prototypes
//
BOOL EnablePropertiesMenu(HWND hwnd, LPSTR pszSel);
...
InitPopupMenus(UINT uMenus, HMENU hMenu, HWND hwndActive)
{
   ...
   if (uMenus & (1 << IDM_FILE)) {
      ...
      pSel = (LPSTR)SendMessage(hwndActive, FS_GETSELECTION, 5, (LPARAM)&bDir);
      ...
      {
         char     szDrv[6];      //  Enough to hold root pathname
         ...
         char szTemp[MAX_PATH + 2 * sizeof(char)];
...
FrameWndProc(HWND hwnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   case WM_FSC:
      ...
      ChangeFileSystem((WORD)wParam, (LPSTR)lParam, NULL);
      break;
   ...
   case WM_WININICHANGE:
      ...
      if (!lParam || !lstrcmpi((LPSTR)lParam, "colors")) {
      ...
      //
      if (lParam && (!lstrcmpi((LPSTR)lParam, "Environment"))) {
         PVOID pEnv;
