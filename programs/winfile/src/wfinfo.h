...
DWORD WINAPI UpdateInit(LPVOID ThreadParameter);
DWORD  WFGetConnection(DRIVE,LPSTR*,BOOL,DWORD);
DWORD GetVolShare(DRIVE drive, LPSTR* ppszVolShare, DWORD dwType);
...
int DocInsert(PPDOCBUCKET ppDocBucket, LPSTR lpszExt, DWORD dwParam);
PDOCBUCKET DocFind(PPDOCBUCKET ppDocBucket, LPSTR lpszExt);
...
LPSTR DocEnum(PDOCENUM pDocEnum, PDWORD pdwParm);
